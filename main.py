...
if use_default == 'n':
    ...
    try:
        specific_combination = [card(type, suit) for type, suit in
                                input("Enter the specific combination (e.g. 9s Ad): ").split()]
    except ValueError:
        print("Invalid input. Using default value of 9s and Ad.")
        specific_combination = [card('9', 's'), card('A', 'd')]

    try:
        simulation_count = int(input("Enter the number of simulations: "))
    except ValueError:
        print("Invalid input. Using default value of 1000 simulation_count.")
        simulation_count = 1000
...
print(
    f"Chance of getting {', '.join(card_to_string(card) for card in specific_combination)} in a {player_count}-player game: {calculate_chance(player_count, cards_in_deck, cards_per_player, specific_combination)}")
print(f"Total Variants: {calculate_variants(player_count, cards_in_deck, cards_per_player)}")
...
winning_odds = simulate_games(deck, specific_combination, player_count, simulation_count)
